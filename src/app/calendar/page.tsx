...
function CalendarContent() {
  const { tasks, updateTask, navigateTo, selectTask } = useTaskContext();
  ...
  const currentChildren = useMemo(
    () =>
      tasks
        .filter(t => t.parentId === currentParentId && t.status !== 'COMPLETED')
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <main className="max-w-5xl mx-auto p-6 space-y-6">
        ...
        <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
          <h3 className="text-base font-semibold text-slate-900 dark:text-white mb-3">Today</h3>
          {todayTasks.length === 0 ? (
            <p className="text-sm text-slate-500 dark:text-slate-400">No tasks scheduled or due today.</p>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {dueToday && (
                        <span className="text-[11px] px-2 py-0.5 rounded-md border text-amber-500 border-amber-500/30 bg-amber-500/10">
                          Due today
                        </span>
                      )}
                    </div>
