...
import { getTopTaskRecommendation, scoreTaskForRecommendation } from '../assistant';
...
export function computeTaskImportance(task: Task): number {
  if (task.status === 'COMPLETED' || task.status === 'ON_HOLD' || task.calendarOnly) {
    return 0;
  }

  return scoreTaskForRecommendation(task);
}
...
export function getSuggestedNextTask(tasks: Task[], _currentParentId: string): Task | null {
  void _currentParentId;
  return getTopTaskRecommendation(tasks);
}
