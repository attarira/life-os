...
export function LocalTray({ items }: { items: LocalTrayItem[] }) {
  ...
  return (
    <div className="mr-1 flex items-center gap-1.5 border-r border-[var(--op-border)] pr-2 font-sans">
      {items.map((item, idx) => {
        if (item.type === 'link') {
          if (item.external) {
            return (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex h-7 w-7 items-center justify-center rounded-md border border-[var(--op-border-strong)] bg-white/[0.03] text-[var(--op-sub)] transition-colors hover:border-[var(--op-accent)] hover:text-[var(--op-accent)]"
                title={item.label}
              >
                {item.icon}
              </a>
          ...
          return (
            <Link
              key={idx}
              href={item.href}
              className="flex h-7 items-center gap-1.5 rounded-md border border-[var(--op-border-strong)] bg-white/[0.03] px-2.5 font-mono text-[10px] uppercase tracking-wider text-[var(--op-sub)] transition-colors hover:border-[var(--op-accent)] hover:text-[var(--op-accent)]"
              title={item.label}
            >
              {item.icon}
        ...
        return (
          <button
            ...
            className="flex h-7 items-center gap-1.5 rounded-md border border-[var(--op-border-strong)] bg-white/[0.03] px-2 text-[var(--op-sub)] transition-colors hover:border-[var(--op-accent)] hover:text-[var(--op-accent)]"
            title={item.label}
          >
            {item.icon}
            {item.badge && (
              <span className="rounded bg-white/[0.06] px-1 py-0.2 font-mono text-[9px] font-semibold text-[var(--op-dim)] tabular-nums">
                {item.badge}
              </span>
            )}
          </button>
